...
const db = require('../controller/controllers')
...
router.get('/product-page/:product_code', async function (req, res) {
	if (!req.params || req.params == undefined) {
		...
		var camInfo = await db.getInfo(req);
		var camFeatures = await db.getFeatures(req);
		var camSpecs = await db.getCamSpecs(req);
		var audioVideo = await db.getAudioVideo(req);
		var automation = await db.getAutomation(req);
		var elecPhys = await db.getElecPhys(req);
		var description = await db.getDesc(req);
